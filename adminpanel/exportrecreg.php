...
$fileName = "Received_Fee_Report" . date('Y-m-d') . ".xls"; 
 
// Column names 
$fields = array('Reg By','Reg No.', 'Student Name', 'Father Name.', 'Email id', 'Batch Name', 'Fee Pack','Pack Fee','Pending Pay Date','Reg By'); 
...
if($query->num_rows > 0){ 
	...
	$sql1="select * from record where regno='".$row['regno']."' ORDER BY id ASC";
        ...
        $lineData = array($row['regby'],$row['regno'], $row['stuname'], $row['fname'], $row['email'], $show1['batchname'], $row['feepack'], $row['packfee'], $row['pendingpaydate'], $show1['regby']); 
...
try {
    ...
    $mail->Subject = 'Full Received Report';
...
unlink($fileName);
echo '<script>window.location.href="feerecreport.php";</script>';
exit;
