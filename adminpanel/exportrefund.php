...
$fileName = "Refund_Fee_Report" . date('Y-m-d') . ".xls"; 
 
// Column names 
$fields = array('Reg By','Reg No.', 'Student Name', 'Father Name.', 'Amount', 'Date','Remarks','Reg By'); 
...
$excelData = implode("\t", array_values($fields)) . "\n"; 
 ...
 $username = $_SESSION['username'];
        ...
        if ($username) {
            $sql="select * from refund where  regby='$username' ORDER BY id ASC";
        } else {
            $sql = "select * from refund ORDER BY id ASC";
        }
...
if($query->num_rows > 0){ 
	...
	$sql1="select * from record where regno='".$row['regno']."' ORDER BY id ASC";
        ...
        $lineData = array($row['regby'],$row['regno'], $row['stuname'], $row['fname'], $row['amt'], $row['date'], $row['remarks'], $show1['regby']); 
...
unlink($fileName);
echo '<script>window.location.href="refundrep.php";</script>';
exit;
