...
if(isset($_POST['Submit'])){
	...
	if(empty($_POST['concession'])){
     ...
     $save2 =mysqli_query($conn,"INSERT into emis(regno,stuname,fname,amt,paydate,mop,tranno,cheqno,status)VALUE('".$reg."','".$stuname."','".$fname."','".$pending."','".$paydate."','".$mop."','".$tranno."','".$cheqno."','0')");
...
//echo "update installments set status='1'  where id='".$id."'";



	$save = "update pendingpay set pendingpaydate ='".$paydate."',paidamt ='".$total."',pendingfee ='".$totalpending."',pendingsts ='".$sts."',status ='0' where regno='".$reg."'";
...
//echo $newFormat;
		  ...
		  <tr style="color: #216b77;font-weight: bold;">
				...
				<td>
			<?php if($show['status']=='0'){?> <button class="btn btn-success" >Pending</button> <?php }else{?>  
<button class="btn btn-danger" >Paid</button>
		...
		<td>
			<?php if($show['status']=='0'){?><a href="concesionamt.php?reg=<?php echo $_GET['reg']; ?>&id=<?php echo $show['id']; ?>" class="btn btn-primary">Fee Concession</a><?php }else{?> 
 <button class="btn btn-danger" >Unable Concession</button>
