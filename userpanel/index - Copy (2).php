...
if(isset($_POST['Submit'])){
    ...
    $sql22 = "SELECT * FROM pack WHERE batchname='$batch_id'";
        ...
        if(mysqli_query($conn, $save)){
            ...
            if($paytype == 'Monthly') {
                   ...
                   for ($i = 0; $i < count($emis); $i++) {
                        ...
                        mysqli_query($conn, "INSERT INTO installments (user_id, batch_id, pack_id, amount, fine, concession, emis, paydate, date, status, paytype, regby) VALUES ('$last_id', '$batch_id', '$packid', '$amountValue', '$fineValue', '$concessionValue', '$emisValue', '$paydateValue', '$date', '0', '$paytype', '" . $_SESSION['user'] . "')");
                    }
...
function showFeePlan(feePayType) {
    ...
    if (feePayType === "Full Advance") {
        ...
        if (noOfMonths && !isNaN(noOfMonths) && noOfMonths > 0) {
            ...
            var table = "<table class='table table-bordered'><tr style='background-color: #216b77;color: white;'><th>Month</th><th>Amount</th><th>Fine</th><th>Concession</th><th>Date</th></tr>";
            for (var i = 1; i <= noOfMonths; i++) {
                table += "<tr><td>" + i + "</td><td><input type='hidden' name='emis[]' value='" + noOfMonths + "'><input type='number' name='amount[]' class='form-control form-control-user' value='" + monthlyAmount.toFixed(2) + "'></td><td><input type='number' name='fine[]' class='form-control form-control-user' value='0'></td><td><input type='number' name='concession[]' class='form-control form-control-user' value='0'></td><td><input type='date' name='paydate[]' class='form-control form-control-user'></td></tr>";
            }
            table += "</table>";
