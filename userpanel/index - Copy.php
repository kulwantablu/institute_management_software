...
if ($result->num_rows > 0) {
  ...
  <strong>Danger!</strong> Reg No. Already Exists!.
	...
	$save ="INSERT into record(regno,batchname,pic,file,stuname,fname,mobno,email,address,highqual,feepack,paytype,packfee,otherqual,category,dob,Installment,regby)VALUE('".$regno."','".$batchname."','".$pic."','".$file."','".$stuname."','".$fname."','".$mobno."','".$email."','".$address."','".$highqual."','".$feepack."','".$paytype."','".$packfee."','".$otherqual."','".$category."','".$dob."','".$Installment."','".$_SESSION['user']."')";
	
if(isset){
	
	mysqli_query($conn,"INSERT into record(regno,batchname,pic,file,stuname,fname,mobno,email,address,highqual,feepack,paytype,packfee,otherqual,category,dob,Installment,regby)VALUE('".$regno."','".$batchname."','".$pic."','".$file."','".$stuname."','".$fname."','".$mobno."','".$email."','".$address."','".$highqual."','".$feepack."','".$paytype."','".$packfee."','".$otherqual."','".$category."','".$dob."','".$Installment."','".$_SESSION['user']."')");
}
...
<div class="col-sm-6">
    ...
    function showFeePlan(feePayType) {
        ...
        if (feePayType == "Full Advance") {
            ...
            var table =
                "<table class='table table-bordered'><tr style='background-color: #216b77;color: white;'><th>Month</th><th>Amount</th><th>Fine</th><th>Concession</th><th>Date</th></tr>";
            for (var i = 1; i <= noOfMonths; i++) {
                table += "<tr><td>" + i +
                    "</td><td><input type='number' name='otherqual'  class='form-control form-control-user' id='bank_date_incorporation' value=" +
                    monthlyAmount +
                    "  ></td><td><input type='number' name='otherqual'  class='form-control form-control-user' id='bank_date_incorporation'  value='0'></td><td><input type='number' name='otherqual'  class='form-control form-control-user' id='bank_date_incorporation'  value='0'></td><td><input type='date' name='otherqual'  class='form-control form-control-user' id='bank_date_incorporation'  ></td></tr>";
            }
            table += "</table>";
            feePlanDiv.innerHTML = table;
        } else if (feePayType == "Installment") {
            var noOfInstallments = prompt("Enter number of installments:");
            var installmentAmount = fee / noOfInstallments;
            var table =
                "<table class='table table-bordered'><tr style='background-color: #216b77;color: white;'><th>Installment No.</th><th>Amount</th><th>Fine</th><th>Concession</th><th>Date</th></tr>";
